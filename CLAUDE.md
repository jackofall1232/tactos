...
| Completion plan (PR #4) | 2026-08-10 | v1 UX polish (search-detail byId fix, toolchain bump AGP 8.13.2/BOM 2026.06.01, saveable nav + BackHandler, edge-to-edge, cold-start surface, transitions + item animations, delete undo via dedup-bypassing restore); lean image toolbox (`core/images` pure-JVM + `feature/images`: resize/convert/compress/EXIF presets ported from ImageToolbox per ADR-0004, SAF-only zero-permission saves, EXIF orientation honored, 24MP budget); Room schema v1 baseline; NOTICE; docs/website truthfulness; CI speedup (dedupe + single invocation + caching) | Local: `:core:*` JVM suites green; CI run 31415157712 green on head 8174470 (assembleDebug + test -x release + lint, APK + schema artifacts); three bot reviews addressed (1 suggestion declined with rationale) | In review (PR #4) |
