...
- **Branch policy** — all work on feature branches (current: 
  `claude/app-polish-website-g6orvq`); no direct commits to `main`; each unit is its own
...
| Supervised build, unit 5 | 2026-07-10 | `core/design` TactosTheme (Material You dynamic color + static teal/amber fallback, light/dark) + ToolboxCard; app shell: ModuleRegistry-driven home grid, state-based navigation, settings skeleton (nothing fakes persistence), DRAFT capture-disclosure screen; `ToolboxModule.emoji` contract addition + test. Zero new dependencies, zero manifest changes | Local: 37 JVM test methods green. CI run #6 (45268d3): compile + `:app:lintDebug` + APK artifact green | In review |
| Supervised build, unit 6 (+ V2 vision) | 2026-07-10 | Owner's V2 "AI workflow engine" vision recorded (todos + binding actions-stay-declarative constraint in memory.md); `feature/clipboard`: timeline UI (search, type/category chips, pinned-first list, favorite, manual add via ContentDetector, detail sheet with copy/share/pin/favorite/delete + category editor), ClipboardToolbox as first contract consumer (gate #4 active), Room encapsulated behind `TactosDb.repository()` | CI run #9 failed (Room supertype on app classpath), fixed by encapsulation; CI run #10 (6eebf86): full pipeline + 7 new feature tests green, APK artifact uploaded | In review |
| Supervised build, v1 polish + website (PR #2) | 2026-07-11 | `core/actions` (ColorValue, JsonTools, QrCode — pure JVM), `core/clipboard` (ShareIngest, SensitiveClips skip-always, ClipboardCapture w/ non-text filtering), v1 contextual actions as ClipAction descriptors + id-keyed executor registry (URL open/QR https-normalized, color HEX⇄RGB⇄HSL/HSV, JSON validate/beautify/minify), DataStore settings + retention (enforced at launch and after every save), first-run onboarding w/ opt-in (default-off) capture disclosure, ACTION_SEND share target (manifest: still zero permissions), adaptive launcher icon, README rewrite, docs/overview.md, zero-external-request website + Pages deploy workflow | Local: JVM suites green (31 new core/actions tests caught 1 real JsonTools bug). 12-agent adversarial review workflow: 8 confirmed findings, all fixed. CI: runs 19/20 + 25/26 failed (Kotlin nested block comment; Robolectric newUri) — both fixed same-session; runs 21–24 + 27/28 green incl. Robolectric sdk=26 sensitive-clip proof; APK artifacts uploaded. Bot reviews: gemini 2 adopted / 1 rebutted with AOSP evidence, codex 5/5 adopted (`.l00prite/reviews/github/pr2-bot-reviews-20260711.md`) | Merged (c5899f7) |
